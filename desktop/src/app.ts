...
const pairServerUrlInput = document.getElementById('pairServerUrlInput') as any;
const pairServerTokenInput = document.getElementById('pairServerTokenInput') as any;
const pairDeviceIdInput = document.getElementById('pairDeviceIdInput') as any;
const pairConnectBtn = document.getElementById('pairConnectBtn') as any;
const pairDisconnectBtn = document.getElementById('pairDisconnectBtn') as any;
const pairCreateBtn = document.getElementById('pairCreateBtn') as any;
...
const pairSessionId = document.getElementById('pairSessionId') as any;
const pairCode = document.getElementById('pairCode') as any;
const pairExpiresAt = document.getElementById('pairExpiresAt') as any;
const pairClaimedUser = document.getElementById('pairClaimedUser') as any;
const pairQrPayload = document.getElementById('pairQrPayload') as any;
...
const I18N = {
  ...
  'en-US': {
    ...
    'pair.hint': 'Connect to the server, create a pair session, then let mobile scan (or copy payload) to build a long-lived remote relationship with this PC.',
...
function getPairServerBaseUrl() {
  const raw = String(pairServerUrlInput?.value || '').trim();
  if (!raw) {
    throw new Error(t('msg.pairNeedServerUrl'));
  }
  ...
  try {
    ...
    throw new Error(t('msg.pairInvalidServerUrl', { url: raw }));
  }
...
function getPairDeviceId() {
  const raw = String(pairDeviceIdInput?.value || '').trim();
  if (!raw) {
    throw new Error(t('msg.pairNeedDeviceId'));
  }
  return raw;
}

function getPairServerToken() {
  return String(pairServerTokenInput?.value || '').trim();
}
...
function buildPairWsUrl(baseUrl, deviceId) {
  ...
  parsed.pathname = '/ws/pc';
  parsed.search = `device_id=${encodeURIComponent(deviceId)}`;
...
function syncPairStorage() {
  ...
  const serverUrl = String(pairServerUrlInput.value || '').trim();
  const serverToken = String(pairServerTokenInput.value || '').trim();
  const deviceId = String(pairDeviceIdInput.value || '').trim();
  if (serverUrl) {
    localStorage.setItem('openclaw.pair.serverUrl', serverUrl);
  }
  localStorage.setItem('openclaw.pair.serverToken', serverToken);
  if (deviceId) {
    localStorage.setItem('openclaw.pair.deviceId', deviceId);
  }
}
...
function updatePairButtons() {
  ...
  const connected = pairWs && pairWs.readyState === WebSocket.OPEN;
  const connecting = pairWs && pairWs.readyState === WebSocket.CONNECTING;
  pairConnectBtn.disabled = connected || connecting;
  pairDisconnectBtn.disabled = !pairWs || pairWs.readyState === WebSocket.CLOSED;
  pairCreateBtn.disabled = connecting;
}
...
function bindPairEnvelope(envelope) {
  if (!envelope || typeof envelope !== 'object') {
    return;
  }

  if (envelope.type === 'pair.claimed') {
    const userId = envelope?.payload?.user_id || envelope?.user_id || '-';
    pairClaimedUser.textContent = userId;
    setPairMessage(t('msg.pairClaimed', { userId }), 'success');
  }
}
...
async function connectPairChannel({ fromReconnect = false } = {}) {
  ...
  ws.onerror = () => {
    appendPairEvent('ws error');
    setPairMessage(t('msg.pairDisconnected'), 'error');
    updatePairButtons();
  };

  ws.onclose = () => {
    appendPairEvent('ws closed');
...
async function createPairSession() {
  ...
  pairSessionId.textContent = data.session_id || '-';
  pairCode.textContent = data.pair_code || '-';
  pairExpiresAt.textContent = data.expires_at || '-';
  pairClaimedUser.textContent = '-';
  pairQrPayload.value = JSON.stringify(data.qr_payload || {}, null, 2);
...
function initPairCenter() {
  ...
  const savedServerUrl = localStorage.getItem('openclaw.pair.serverUrl');
  const savedServerToken = localStorage.getItem('openclaw.pair.serverToken');
  const savedDeviceId = localStorage.getItem('openclaw.pair.deviceId');
  pairServerUrlInput.value = (savedServerUrl || 'http://127.0.0.1:38089').trim();
  pairServerTokenInput.value = (savedServerToken || '').trim();
  pairDeviceIdInput.value = (savedDeviceId || createDefaultPairDeviceId()).trim();
  syncPairStorage();

  pairSessionId.textContent = '-';
  pairCode.textContent = '-';
  pairExpiresAt.textContent = '-';
  pairClaimedUser.textContent = '-';
  pairQrPayload.value = '';
  ...
  pairServerUrlInput.addEventListener('change', () => {
    syncPairStorage();
  });
  pairServerTokenInput.addEventListener('change', () => {
    syncPairStorage();
  });
  pairDeviceIdInput.addEventListener('change', () => {
    syncPairStorage();
  });
  ...
  pairDisconnectBtn.addEventListener('click', () => {
...
langSelect.addEventListener('change', async () => {
  ...
  if (isPairCenterAvailable()) {
    if (pairWs && pairWs.readyState === WebSocket.OPEN) {
      renderPairWsStatus('connected');
    } else if (pairWs && pairWs.readyState === WebSocket.CONNECTING) {

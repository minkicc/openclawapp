  ...
  async function handleScan(rawValue: string) {
    ...
    try {
      ...
      const body = result.created
        ? `已创建会话“${result.session.name}”。${result.session.safetyCode ? ` 请在桌面端确认安全码 ${result.session.safetyCode}。` : ''}`
        : `已找到现有会话“${result.session.name}”。${result.session.safetyCode ? ` 当前安全码 ${result.session.safetyCode}。` : ''}`;
